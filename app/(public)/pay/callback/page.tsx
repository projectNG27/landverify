...
function pickPaystackReference(sp: { reference?: string | string[]; trxref?: string | string[] } | null | undefined): string {
  const safe = sp ?? {};
  ...
  const merged = one(safe.trxref) || one(safe.reference) || "";
  const m = merged.match(/^(LVPAY-[A-F0-9]+)/i);
  return m ? m[1].toUpperCase() : merged.trim();
}

/** Intake pay-first: redirect in plain HTML so we never rely on `next/navigation` redirect() or a client-only RSC child (both have caused generic Vercel "Application error" for some users). */
function IntakePayReturnHtml({ paymentRef }: { paymentRef: string }) {
  const href = `/submit-request?payment_ref=${encodeURIComponent(paymentRef)}`;
  return (
    <div className="mx-auto max-w-lg px-4 py-16 text-center">
      <script
        // Ref is server-validated `LVPAY-*` from Paystack; JSON.stringify makes the injected URL safe.
      ...
      <p className="mt-3 text-sm text-[var(--lv-ink-muted)]">Taking you back to the request form…</p>
...
export default async function PayCallbackPage({ searchParams }: Props) {
  try {
    ...
    if (result.ok && result.intakeCheckout) {
      return <IntakePayReturnHtml paymentRef={reference} />;
    }
    ...
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <h1 className="text-xl font-bold text-[var(--lv-ink)]">Could not finish payment return</h1>
        <p className="mt-3 text-sm text-[var(--lv-ink-muted)]">
          Something went wrong on our server after Paystack. If you were charged, use Track request in a minute or contact
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4 text-sm font-semibold text-[var(--lv-primary)]">
          ...
          <Link href="/submit-request" className="hover:underline">
            Submit request
          </Link>
        </div>
      </div>
